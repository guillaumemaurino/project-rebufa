...
function change_participants(participants_id) {
  var change_participants_description = document.getElementById("change_participants_description");
  var change_participants_conditions = document.getElementById("change_participants_conditions");

  if(event.keyCode == 13) {
    const ul = document.getElementById('participantss_js');
    const url = '/participantss/' + participants_id;
    console.log(url);
      ...
      .then((resp) => resp.json())
      .then(function(data) {
            console.log('Front end - get participants:')
            console.log(data);
            // Here we should do a put to update the name !!!!
            if (change_participants_description.value != ""){
              data.description = change_participants_description.value;
              console.log('Changing participants descriptions');
            }
            if (change_participants_conditions.value != ""){
              data.conditions = change_participants_conditions.value;
              console.log('Changing participants conditions');
            }
            var request = new Request(url, {
              method: 'PUT',
